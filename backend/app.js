...
const isadmin = async (req, res, next) => {
  try {
    ...
    if (user.email === "haraprasadmahapatra223@gmail.com") {
      return next();
    }
...
app.get("/checkfollow/:authorid", verifyJWT, async (req, res) => {
  ...
  try {
    ...
    if (!isfollow) {
      return res
        .status(200)
        .json({ success: true, isfollow: 0, authorname: author.name });
    } else {
      return res
        .status(200)
        .json({ success: true, isfollow: 1, authorname: author.name });
    }
  } catch (err) {
    console.log(err);
    return res.status(500).json({ message: "internal server error" });
...
app.get("/getnotifications", verifyJWT, async (req, res) => {
  ...
  try {
    let notifications = await Notification.find({ receipt: req.user.id })
      .populate("sender")
      .sort({ createdAt: -1 });
    return res
      .status(200)
...
app.post("/updateprofile", verifyJWT, async (req, res) => {
  try {
    ...
    let updateduser = await User.updateOne(
      { _id: req.user.id },
      { $set: { username: username, avatarurl: avatar } }
    );

    if (!updateduser) {
      return res
        .status(400)
        .json({ success: false, message: "could not update user's profile" });
    }

    return res
      .status(200)
      .json({ success: true, message: "updated user's profile" });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
...
app.get("/getThought/:postid", verifyJWT, async (req, res) => {
  ...
  try {
    ...
    if (!post) {
      return res
        .status(404)
        .json({ success: false, message: "could not find the post" });
    }
  ...
  try {
    ...
    if (profiles.length == 0) {
      return res
        ...
        .json({
          success: true,
          profiles,
          message: "no followings of the given id",
        });
    }
    ...
    return res
      ...
      .json({
        success: true,
        profiles,
        message: " followings of the given id ",
      });
  } catch (err) {
    console.log(err);
    return res
  ...
  try {
    ...
    if (profiles.length == 0) {
      return res
        ...
        .json({
          success: true,
          profiles,
          message: "no followers of the given id",
        });
    }
    ...
    return res
      ...
      .json({
        success: true,
        profiles,
        message: " followers of the given id ",
      });
  } catch (err) {
    console.log(err);
    return res
...
app.get("/getlikes/:postid", verifyJWT, async (req, res) => {
  ...
  try {
    ...
    if (likes.length == 0) {
      return res
        .status(200)
        .json({ success: true, message: "No likes in this post", likes });
    }

    return res
      .status(200)
      .json({ success: true, message: "profiles found", likes });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
...
app.delete("/deletepost/:postid", verifyJWT, async (req, res) => {
  try {
...
//1.) route to delete any user
app.delete("/deleteuser/:userid", verifyJWT, isadmin, async (req, res) => {
  ...
  if (!user) {
    return res
      .status(404)
      .json({ message: "the user do not exist", success: false });
  }
  ...
  try {
    await Promise.all([
      ...
      Follow.deleteMany({ follower: userid }, { session }),
      Follow.deleteMany({ following: userid }, { session }),
      Notification.deleteMany({ sender: userid }, { session }),
      Notification.deleteMany({ receipt: userid }, { session }),
      Post.deleteMany({ author: userid }, { session }),
    ]);
